...
const SUPARBASE_PUBLIC_KEY = "sb_publishable_nSpXT2pPiiQCLKisCh60mw_6YBSr4cX";
...
// Check if specific tables exist in Supabase schema cache before querying
async function verifySupabaseTables(): Promise<boolean> {
  try {
    ...
    if (settingsError) {
      if (
        ...
        settingsError.message?.toLowerCase().includes("schema cache") ||
    ...
    if (usersError) {
      if (
        ...
        usersError.message?.toLowerCase().includes("schema cache") ||
        usersError.message?.toLowerCase().includes("relation")
      ) {
        return false;
      }
    }
    ...
    console.warn("[SUPABASE] Table validation query failed:", err.message);
    return false;
  }
}
...
async function syncFromSupabase() {
  try {
    console.log("[SUPABASE SYNC] Verifying remote database schema availability...");
    isSupabaseTableAvailable = await verifySupabaseTables();

    if (!isSupabaseTableAvailable) {
      console.log("[SUPABASE] Remote table schema is not initialized or accessible in this Supabase database. Operating seamlessly in secure local storage mode fallback.");
      // Seamlessly populate database cache with local model
      readDb();
    ...
    if (userError) {
      console.warn("[SUPABASE SYNC] Skip/Warning on system user bootstrap (could be missing/different schema):", userError.message);
    }
    ...
    console.warn("[SUPABASE SYNC] Sync error. Falling back to local/memory file caching storage:", err.message);
  }
}

async function syncToSupabase(data: any) {
  if (!isSupabaseTableAvailable) {
    // Graceful silent return if tables are not available on remote DB
    return;
  }
  ...
  try {
    ...
    if (error) {
      console.warn("[SUPABASE SYNC] Error flushing to cloud table user_settings:", error.message);
    } else {
      console.log("[SUPABASE SYNC] Database flushed to Supabase cloud storage safely.");
    }
  } catch (err: any) {
    console.error("[SUPABASE SYNC] Critical exception in cloud flush:", err.message);
  } finally {
...
async function generateContentWithFallback(
  ...
  // Cleanly map outdated or deprecated names to modern ones
  let mappedModel = primaryModel;
  const lowerModel = (primaryModel || "").toLowerCase().trim();
  if (lowerModel === "gemini-1.5-pro" || lowerModel === "gemini-2.0-pro") {
    mappedModel = "gemini-3.5-flash";
  } else if (lowerModel === "gemini-1.5-flash" || lowerModel === "gemini-2.0-flash") {
    mappedModel = "gemini-3.5-flash";
  }

  const modelsToTry = [mappedModel];
