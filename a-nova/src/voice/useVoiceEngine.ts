...
export function useVoiceEngine({ onSendMessage, onTranscriptChange, requestPermission }: UseVoiceEngineProps = {}) {
  ...
  // Stop Session & Return to Idle (Triggered when user taps stop)
  const stopVoiceSession = useCallback(() => {
    const finalSttText = cleanupVoiceSession();
    stateMachineRef.current.transitionTo("IDLE");

    const textToSet = (finalSttText || currentTranscriptRef.current || "").trim();
    if (textToSet) {
      setInputText(textToSet);
      onTranscriptChange?.(textToSet);
    }
  }, [cleanupVoiceSession, onTranscriptChange]);
  ...
  const submitTranscript = useCallback(
    async (rawText?: string) => {
      ...
      const textToSubmit = rawText !== undefined ? rawText : (inputText || currentTranscriptRef.current);
    ...
    [cleanupVoiceSession, inputText, onSendMessage]
  );
  ...
  const startListeningSession = useCallback(async () => {
    ...
    const performStart = async () => {
      ...
      vadRef.current.start(stream, {
        ...
        onSilenceDetected: () => {
          // Silence detected -> stop listening, keep transcript in composer for manual sending
          stopVoiceSession();
        },
      });
  ...
  }, [cleanupVoiceSession, onTranscriptChange, requestPermission, stopVoiceSession]);
  ...
  const toggleListening = useCallback(() => {
    ...
    if (currentState === "LISTENING") {
      // User tapped stop -> stop listening, keep transcript in composer
      stopVoiceSession();
