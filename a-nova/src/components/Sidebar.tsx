...
interface SidebarProps {
  ...
  onNewSession: (mode?: 'general' | 'math' | 'coding') => void;
  ...
  onToggleMobile: () => void;
  settings?: any;
  onSetChatMode?: (id: string, mode: 'general' | 'math' | 'coding') => void;
  activeMode?: 'general' | 'math' | 'coding';
  onSelectMode?: (mode: 'general' | 'math' | 'coding') => void;
  onOpenLogin?: () => void;
}
...
const Sidebar = React.memo(function Sidebar({
  ...
  const activeMode = propActiveMode || activeSession?.mode || 'general';
  ...
  const handleCoreClick = (mode: 'general' | 'math' | 'coding') => {
    try {
    ...
    if (onSelectMode) {
      ...
      if (activeSessionId) {
        if (onSetChatMode) {
          onSetChatMode(activeSessionId, mode);
        }
      } else {
        onNewSession(mode);
      }
  ...
  // Filter and group conversations efficiently
  const { visibleSessions, pinnedSessions, regularSessions, groupedSessions } = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    const filtered = sessions.filter(s => {
      const titleMatch = (s.title || "Untitled Chat").toLowerCase().includes(query);
      const modeMatch = (s.mode || 'general') === activeMode;
  ...
  }, [sessions, searchQuery, activeMode]);
  ...
  return (
    <>
      ...
      <aside
        id="side_navigation"
        className={`fixed lg:sticky top-0 left-0 bottom-0 z-50 flex flex-col h-screen h-[100dvh] transition-all duration-300 ease-out lg:translate-x-0 ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } ${isCollapsed ? "lg:w-16 w-60 max-w-[78vw]" : "lg:w-68 w-64 sm:w-72 max-w-[80vw]"} ${
          isDark 
      ...
      >
        ...
        <div className={`p-2.5 sm:p-3 md:p-4 flex items-center justify-between border-b h-10 sm:h-12 md:h-15 shrink-0 ${
          isDark ? "border-zinc-900" : "border-zinc-200"
        }`}>
          {!isCollapsed ? (
            <AnovaLogo size="sm" showText={true} subtitle="INTELLIGENCE" animated={false} />
          ) : (
            <div className="mx-auto">
              <AnovaLogo size="sm" showText={false} animated={false} />
            </div>
          )}
          ...
          <button 
            type="button"
            onClick={onToggleMobile} 
            className={`lg:hidden p-1 sm:p-1.5 rounded-lg cursor-pointer transition-colors ${
              isDark ? "text-zinc-400 hover:text-white hover:bg-zinc-900" : "text-zinc-550 hover:text-zinc-900 hover:bg-zinc-150"
            }`}
          >
            <X className="w-4 h-4" />
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-2.5 md:p-3 space-y-2 sm:space-y-3 md:space-y-4">
          ...
          <div>
            <button
              ...
              className={`w-full group relative flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold rounded-xl border transition-all cursor-pointer ${
          ...
          <div className="space-y-1">
            ...
            {CORES.map((core) => {
              ...
              const isActive = activeMode === core.id;
              ...
              return (
                <button
                  ...
                  className={`w-full group relative flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer text-xs transition-all ${
                    isActive
          ...
          {!isCollapsed && (
            <div className="relative w-full">
              <input
                ...
                className={`w-full pl-9 pr-8 py-2.5 text-xs rounded-xl outline-none transition-all ${
                  isDark 
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-400 transition-colors cursor-pointer"
                >
              ...
              {orderedGroups.map(group => {
                ...
                return (
                  <div key={group} className="space-y-1">
                    {!isCollapsed && group !== "Previous 7 Days" && (
        ...
        {!user ? (
          <div className={`p-3 border-t mt-auto shrink-0 ${
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-2 px-3 rounded-full font-semibold text-xs transition-all duration-200 cursor-pointer border ${
                isDark
          ...
          <div className={`p-2.5 border-t mt-auto shrink-0 ${
            isDark ? "border-zinc-900 bg-zinc-950/80" : "border-zinc-200 bg-zinc-50/80"
          }`}>
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`flex-1 group relative flex items-center justify-center gap-1.5 p-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              ...
              <button
                ...
                type="button"
                onClick={onLogout}
                className={`group relative flex items-center justify-center p-2 rounded-xl transition-all cursor-pointer ${
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs z-999 animate-fade-in text-white select-none">
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                   <h3 className="text-sm font-bold font-display">Delete Chat</h3>
                  <p className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                    Are you sure you want to delete this chat? You cannot undo this.
                  </p>
                </div>
              </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs z-999 animate-fade-in text-white select-none">
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                   <h3 className="text-sm font-bold font-display">Delete {selectedIds.length} Chats</h3>
                  <p className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                    This will permanently delete the {selectedIds.length} selected chats. You cannot undo this.
                  </p>
                </div>
              </div>
              <div className="flex justify-end gap-2">
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs z-999 animate-fade-in text-white select-none">
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                   <h3 className="text-sm font-bold font-display">Delete All History</h3>
                  <p className="text-[11px] text-zinc-400 mt-1 leading-relaxed">
                    This will permanently delete ALL chats from your account history. You cannot undo this.
                  </p>
                </div>
              </div>
              <div className="flex justify-end gap-2">
    ...
    return (
      ...
      >
        <button
          ...
          className={`w-full relative flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left cursor-pointer text-xs transition-all group-hover:pr-18 ${
            isActive
              ? isDark 
