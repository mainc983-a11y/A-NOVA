...
app.post("/api/chats", authenticate, (req, res) => {
  ...
  const userSettings = db.settings[user.id] || { defaultModel: "gemini-2.5-flash" };
  ...
  const newChat = {
    ...
    selectedModel: userSettings.defaultModel || "gemini-2.5-flash",
    messages: [],
    mode: mode || "general",
    createdAt: new Date().toISOString(),
...
app.post("/api/chats/:id/message", authenticate, async (req, res) => {
  ...
  const userSettings = db.settings[user.id] || { defaultModel: "gemini-2.5-flash", systemPrompt: "" };
  const modelToUse = chat.selectedModel || userSettings.defaultModel || "gemini-2.5-flash";
