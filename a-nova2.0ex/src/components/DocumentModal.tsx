          ...
          {mode === "preview" ? (
                    ...
                    <div className="space-y-4">
                      ...
                      >
                        ...
                        <div className="my-auto py-4 space-y-3">
                          {slides[activeSlideIdx]?.bulletPoints?.map((bp, i) => (
                            <div key={`bp_${i}`} className="flex items-start gap-3 text-sm sm:text-base">
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-1.5 overflow-x-auto py-1 max-w-md">
                          {slides.map((s, idx) => (
                            <button
                              key={s.id ? `s_${s.id}_${idx}` : `slide_${idx}`}
                              type="button"
                ...
                >
                  ...
                  <div className="space-y-8">
                    {doc.sections?.map((sec, secIdx) => (
                      <div key={sec.id ? `sec_${sec.id}_${secIdx}` : `sec_idx_${secIdx}`} className="space-y-3">
                        {sec.title && (
                            ...
                            <table className="w-full text-left text-xs border-collapse">
                              <thead>
                                <tr className="bg-zinc-100 dark:bg-zinc-800/90 text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-700">
                                  {sec.tableHeaders.map((th, thi) => (
                                    <th key={`th_${thi}`} className="px-4 py-2.5 font-bold">
                                      {th}
                                    </th>
                                  ))}
                                </tr>
                              ...
                              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
                                {sec.tableRows.map((tr, tri) => (
                                  ...
                                  >
                                    {tr.map((tc, tci) => (
                                      <td key={`tc_${tri}_${tci}`} className="px-4 py-2 text-zinc-700 dark:text-zinc-300">
                                        {tc}
                                      </td>
                                    ))}
                                  </tr>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="space-y-4">
                ...
                {doc.sections.map((sec, secIdx) => (
                  <div
                    key={sec.id ? `sec_edit_${sec.id}_${secIdx}` : `sec_edit_${secIdx}`}
                  ...
                  >
                    ...
                    <div className="space-y-2 pt-2 border-t border-zinc-200 dark:border-zinc-800">
                      ...
                      {sec.bullets?.map((bullet, bulletIdx) => (
                        <div key={`bullet_edit_${sec.id || secIdx}_${bulletIdx}`} className="flex items-center gap-2">
