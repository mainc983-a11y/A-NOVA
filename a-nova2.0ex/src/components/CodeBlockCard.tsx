...
import { Check, Copy, Download, Terminal, FileCode, Database, Braces, WrapText, Hash } from "lucide-react";
...
function tokenizeLine(line: string, isDark: boolean) {
  ...
  // Tokenize words, strings, numbers, punctuation
  const tokenRegex = /(".*?"|'.*?'|`.*?`|\/\/.+$|# .+$|\b\d+(?:\.\d+)?\b|\b(?:const|let|var|function|return|import|export|from|default|class|if|else|for|while|async|await|def|public|private|protected|static|interface|type|enum|select|where|insert|update|delete|from|join|group|by|order|having|limit|true|false|null|undefined|None|self|this)\b|[{}()\[\];,])/g;
  ...
  while ((match = tokenRegex.exec(line)) !== null) {
    ...
    } else if (["const", "let", "var", "function", "return", "import", "export", "from", "class", "if", "else", "for", "while", "async", "await", "def", "select", "where"].includes(token)) {
...
export const CodeBlockCard: React.FC<CodeBlockCardProps> = ({
  ...
  const handleDownloadCode = () => {
    try {
      ...
      link.href = url;
      link.download = `code_snippet_${Date.now()}${cfg.ext}`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      setToastMsg(`Saved code as code_snippet${cfg.ext}`);
  ...
  return (
    <div className={`my-3.5 w-full rounded-2xl border transition-all duration-300 shadow-md hover:shadow-lg overflow-hidden font-mono relative group ${
      ...
      {/* Toast Popup */}
      {toastMsg && (
        <div className="absolute top-2.5 left-1/2 -translate-x-1/2 z-20 bg-emerald-500 text-white text-[11px] px-3 py-1 rounded-full shadow-lg font-sans font-medium animate-fadeIn">
          {toastMsg}
        </div>
      )}
      ...
      <div className="px-3.5 py-2.5 bg-zinc-900/90 border-b border-zinc-800 flex flex-wrap items-center justify-between gap-2 text-xs select-none">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition text-[11px] flex items-center gap-1 cursor-pointer ${
              showLineNumbers
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-400 hover:text-emerald-400 transition text-[11px] flex items-center gap-1 cursor-pointer"
      ...
      <div className="relative overflow-x-auto max-h-[460px] p-3 sm:p-4 text-[11.5px] sm:text-[12.5px] leading-relaxed font-mono select-text">
        <div className="flex">
          {/* Line Numbers Gutter */}
          {showLineNumbers && (
            <div className="pr-3.5 mr-3 border-r border-zinc-800/80 text-zinc-600 font-mono text-right select-none shrink-0 space-y-0.5">
              {lines.map((_, idx) => (
                <div key={idx} className="h-5 text-[10.5px]">
                  {idx + 1}
                </div>
              ))}
            </div>
          ...
          <div className={`min-w-0 flex-1 space-y-0.5 ${wrapLines ? "whitespace-pre-wrap break-all" : "whitespace-pre"}`}>
            {lines.map((line, lIdx) => (
              <div key={lIdx} className="h-5 flex items-center">
                {tokenizeLine(line, true)}
              </div>
            ))}
          </div>
        </div>
