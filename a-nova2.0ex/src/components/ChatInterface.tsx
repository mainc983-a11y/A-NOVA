...
const CodeBlock = memo(function CodeBlock({
  ...
  return (
    <div className={`my-4 rounded-xl border overflow-hidden font-mono shadow-sm ${
      ...
      <pre className="p-3 sm:p-4 overflow-x-auto text-[11px] sm:text-[12px] leading-relaxed font-mono max-h-96 select-text max-w-full">
...
const MessageBubble = memo(function MessageBubble({
  ...
  return (
    <div className="py-0.5 sm:py-1 md:py-2 px-2 sm:px-3 md:px-4 transition-colors w-full flex justify-center">
      <div className={`w-full min-w-0 flex gap-1.5 sm:gap-2 md:gap-3 ${
        ...
        {isAssistant ? (
          <div className="w-[22px] h-[22px] sm:w-[24px] sm:h-[24px] md:w-[26px] md:h-[26px] rounded-full bg-gradient-to-tr from-sky-505 via-indigo-505 to-purple-605 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
        ...
        <div className={`flex flex-col max-w-[92%] sm:max-w-[85%] md:max-w-[78%] space-y-0.5 md:space-y-1 min-w-0 ${
          isAssistant ? "items-start" : "items-end"
        }`}>
          
          {/* Bubble metadata layer */}
          <div className="flex items-center gap-1.5 px-1 text-[10px] text-zinc-400 dark:text-zinc-500 font-sans select-none pb-0 md:pb-0.5">
            {isAssistant && (
              <>
          ...
          <div className={`px-2.5 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2.5 mt-0.5 rounded-2xl border text-left shadow-none max-w-full overflow-hidden break-words ${
            isAssistant
              ? isDark 
                ? "bg-zinc-900/40 border-zinc-800 text-zinc-150" 
                : "bg-zinc-50/80 border border-zinc-200/80 text-zinc-900"
              : isDark
                ? "bg-zinc-805/90 border-zinc-750 text-zinc-50"
                : "bg-zinc-100 border border-zinc-200/80 text-zinc-900"
          }`}>
            <div className="select-text max-w-full overflow-hidden break-words">
              {isAssistant ? (
                parseAndRenderMarkdown(msg.content, msg.id, copiedCodeId, copyTextToClipboard, isDark, fontSize)
              ) : (
                <p className={`whitespace-pre-wrap ${dynamicFsClass} leading-relaxed font-sans break-words`}>{msg.content}</p>
              )}
            </div>
          </div>
...
const ChatInterface = React.memo(function ChatInterface({
  ...
  const AVAILABLE_MODELS = [
    { id: "gemini-2.5-flash", name: "A-Nova Core (Fastest)", tag: "2.5 Flash", default: true },
    { id: "gemini-2.5-pro", name: "Complexity Reasoner", tag: "2.5 Pro" },
    { id: "gemini-2.5-flash-lite", name: "Logical Standard", tag: "2.5 Lite" }
  ];

  const currentModelStr = settings?.defaultModel || "gemini-2.5-flash";
  ...
  return (
    <div 
      ...
      onDrop={handleDrop}
      className={`flex-1 w-full max-w-full min-w-0 flex flex-col h-[100dvh] lg:h-screen relative transition-colors duration-300 ${
    ...
    >
      ...
      <header className={`h-10 sm:h-11 md:h-13 w-full max-w-full flex items-center justify-between px-2 sm:px-3 md:px-4 border-b shrink-0 ${
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
          {onToggleSidebar && (
            <button
              ...
              className={`lg:hidden p-1 sm:p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors cursor-pointer ${
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2">
            <span className="text-[10px] sm:text-[11px] md:text-xs font-semibold select-none flex items-center gap-1 sm:gap-1.5 px-1.5 py-0.5 sm:px-2 sm:py-0.5 md:px-2.5 md:py-1 rounded-lg bg-zinc-100 dark:bg-zinc-900 tracking-wide font-display">
      ...
      <div 
        ...
        className="flex-1 w-full max-w-full min-w-0 overflow-y-auto selection:bg-sky-500/20"
      >
        {!session || !session.messages || session.messages.length === 0 ? (
          /* Unified Stately Welcome Empty State across all chat modes */
          <div className="w-full max-w-xl mx-auto px-2.5 sm:px-4 py-3 sm:py-6 md:py-12 flex flex-col items-center justify-center text-center space-y-2.5 sm:space-y-4 md:space-y-6 select-none my-auto">
            <motion.div
            ...
            >
              <AnovaLogo size="lg" showText={true} subtitle={activePreset.title.toUpperCase()} animated={true} />
            </motion.div>

            <div className="space-y-1.5 md:space-y-2 max-w-md w-full">
            ...
            {!session && (
              <div className="pt-1 md:pt-2">
                <span className="px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[9px] sm:text-[10px] font-mono">
          ...
          <div className="w-full max-w-full min-w-0 py-0.5 md:py-2">
            ...
            {/* Simplest typing indicator */}
            {loading && (
              <div className="py-1 sm:py-1.5 md:py-2.5 px-2 sm:px-3 md:px-4 w-full flex justify-center">
                <div className={`w-full flex gap-3 ${
                  settings?.chatWidth === "full" ? "max-w-5xl" : "max-w-3xl"
                } justify-start`}>
                  <div className="w-[22px] h-[22px] sm:w-[24px] sm:h-[24px] md:w-[26px] md:h-[26px] rounded-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                  ...
                  <div className="flex flex-col space-y-1 items-start max-w-[85%]">
                    <div className="flex items-center gap-1.5 px-1 text-[10px] text-zinc-400 dark:text-zinc-500 font-sans select-none pb-0.5">
                      <span className="font-semibold text-zinc-750 dark:text-zinc-300">A-Nova</span>
                    </div>
                    <div className={`px-3 py-1.5 sm:px-4 sm:py-2 mt-0.5 rounded-2xl border text-left text-xs bg-zinc-50 dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-450 shadow-none`}>
                      <div className="flex items-center gap-1.5">
                        <span className="text-zinc-550 dark:text-zinc-400 font-medium">A-Nova is typing...</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* Unified Input area */}
      <footer className={`p-1.5 sm:p-2.5 md:p-4 border-t shrink-0 pb-safe w-full max-w-full min-w-0 ${
        isDark ? "bg-zinc-950/40 border-zinc-900" : "bg-zinc-50/30 border-zinc-150"
      }`}>
        <div className="w-full max-w-2xl sm:max-w-3xl px-2 sm:px-3 mx-auto space-y-1.5 sm:space-y-2 md:space-y-3">
          ...
          <form 
            ...
            className={`relative rounded-3xl border transition-all duration-200 shadow-sm focus-within:shadow-md ${
              isDark 
                ? "bg-zinc-900/90 border-zinc-800 focus-within:border-purple-500/50" 
                : "bg-white border-zinc-200 focus-within:border-purple-500/50"
            }`}
          >
            {/* Active search parameters alert indicator */}
            {webSearchActive && (
              <div className="flex items-center gap-1.5 px-3.5 sm:px-4.5 pt-2.5 select-none text-emerald-500 text-[10px] sm:text-[11px] font-bold font-mono">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5 px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2.5">
              ...
              <div className="flex items-center justify-center shrink-0">
                <button
                  ...
                  className={`flex h-8.5 w-8.5 sm:h-9.5 sm:w-9.5 md:h-10.5 md:w-10.5 items-center justify-center rounded-full transition-all cursor-pointer ${
                    isPlusMenuOpen 
                  ...
                  title="Attach & Tools"
                >
                  <Plus className={`w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5.5 md:h-5.5 transition-transform duration-200 ${isPlusMenuOpen ? "rotate-45" : ""}`} />
                </button>
              </div>
              ...
              <textarea
                ...
                placeholder={`Message A-NOVA (${activePreset.title})...`}
                disabled={loading}
                value={inputText}
                ...
                className="w-full bg-transparent px-2 py-1 sm:py-1.5 md:py-2 text-[16px] sm:text-[17px] md:text-[17px] leading-relaxed focus:outline-none placeholder-zinc-400 dark:placeholder-zinc-500 font-sans resize-none block flex-1 max-h-40 min-h-[2rem] sm:min-h-[2.25rem] md:min-h-[2.5rem] overflow-y-auto"
              />

              {/* Action Buttons Group (Voice Mic / Send + Voice Note) */}
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                {/* Voice Dictation (Mic) button or Send button */}
                {(!inputText.trim() && attachedFiles.length === 0) ? (
                  ...
                  >
                    {isListening ? <MicOff className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5" /> : <Mic className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5" />}
                  </button>
                ) : (
                  <button
                    type="submit"
                    disabled={loading || (!inputText.trim() && attachedFiles.length === 0)}
                    className="flex h-8.5 w-8.5 sm:h-9.5 sm:w-9.5 md:h-10.5 md:w-10.5 items-center justify-center rounded-full bg-purple-600 hover:bg-purple-550 text-white shadow-md transition-all cursor-pointer disabled:opacity-50"
                    title="Send message"
                  >
                    {loading ? <Loader2 className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5 animate-spin" /> : <ArrowUp className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5 stroke-[2.5]" />}
                  </button>
                )}

                {/* Voice note / Live audio trigger button */}
                <button
                  type="button"
                  ...
                  title="Voice note / Live recorder"
                >
                  <Volume2 className="w-4.5 h-4.5 sm:w-5 sm:h-5 md:w-5.5 md:h-5.5" />
                </button>
              </div>
            </div>

            {/* Floating Popover on Desktop + Bottom Sheet on Mobile for Attachment/Tools menu */}
            <AnimatePresence>
              {isPlusMenuOpen && (
                <>
                  {/* Backdrop overlay */}
                  ...
                  <motion.div
                    ...
                    className={`z-50 select-none ${
                      "max-sm:fixed max-sm:inset-x-0 max-sm:bottom-0 max-sm:w-full max-sm:rounded-t-2xl max-sm:p-3 " +
                      "sm:absolute sm:left-2 sm:bottom-16 sm:w-52 sm:rounded-2xl sm:p-2 sm:shadow-xl " +
                      (isDark 
                  ...
                  >
                    ...
                    <div className="sm:hidden w-8 h-1 rounded-full bg-zinc-700/60 mx-auto mb-2" />
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all cursor-pointer text-left ${
                          isDark 
                            ? "hover:bg-zinc-800 active:bg-zinc-750 text-zinc-100" 
                            : "hover:bg-zinc-100 active:bg-zinc-200 text-zinc-800"
                        }`}
                      >
