...
export default function StorageAndFilesTab({
  ...
  const [clearConfirmType, setClearConfirmType] = useState<"all" | "files" | "images" | "batch" | null>(null);
  ...
  const handleDownloadItem = (item: StorageItem) => {
    try {
      if (!item.dataUrl) {
        showErrorNotification("File data URL is missing.");
        return;
      }
  ...
  const handleClearStorage = (type: "all" | "files" | "images") => {
    if (!onUpdateSessions) return;

    const updatedSessions = sessions.map((sess) => {
      const newMessages = sess.messages.map((msg) => {
        let newAttached = msg.attachedFiles;
        let newGen = msg.generatedImages;
    ...
    showSuccessNotification(
      type === "all"
        ? "Cleared all storage and attachments"
        : type === "images"
        ? "Cleared all AI-generated images"
        : "Cleared all uploaded file attachments"
    );
  };
  ...
  const handleExportDataBackup = async () => {
    ...
    setTimeout(() => {
      ...
      const backupData = {
        ...
        totalStorageItems: allStorageItems.length,
        totalUsedStorageBytes: totalUsedBytes,
        sessions: sessions,
        storageItemsSummary: allStorageItems.map((i) => ({
          name: i.fileName,
          type: i.fileType,
          size: i.fileSize,
          category: i.category,
          conversation: i.sessionTitle,
          date: i.date
        }))
      };
      ...
      downloadAnchor.setAttribute("download", `A_NOVA_Account_Backup_${new Date().toISOString().slice(0, 10)}.json`);
      ...
      setExportProgress(0);
      showSuccessNotification("Account backup exported successfully!");
    }, 900);
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-6">
      ...
      {/* 5. EXPORT & CLEAR STORAGE CONTROLS */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-zinc-200 dark:border-zinc-800/80">
        ...
        <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100">Export Backup & Files</h4>
          </div>
          <p className="text-[11px] text-zinc-600 dark:text-zinc-400">
            Download a full account backup including conversation history, settings, and file details.
          </p>
          ...
          <button
            ...
            className="w-full py-2.5 px-4 bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-700/80 text-xs font-semibold rounded-xl flex items-center justify-center gap-2 text-zinc-800 dark:text-zinc-200 cursor-pointer transition disabled:opacity-50"
          >
            {isExporting ? (
              <>
        ...
        <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100">Clear Storage</h4>
          </div>
          <p className="text-[11px] text-zinc-600 dark:text-zinc-400">
            Permanently remove attachments or AI generated images to free up space instantly.
          </p>

          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => setClearConfirmType("files")}
              className="py-2 px-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/20 text-xs font-bold rounded-xl transition cursor-pointer text-center"
            >
              Clear Files
            </button>
            <button
              type="button"
              onClick={() => setClearConfirmType("images")}
              className="py-2 px-3 bg-purple-500/10 hover:bg-purple-500/20 text-purple-600 dark:text-purple-400 border border-purple-500/20 text-xs font-bold rounded-xl transition cursor-pointer text-center"
            >
              Clear Images
            </button>
          </div>
      ...
      {/* E. Clear / Batch Delete Confirmation */}
      <AnimatePresence>
        {clearConfirmType && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
            <motion.div
            ...
            >
              ...
              <div>
                <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">
                  {clearConfirmType === "batch"
                    ? `Delete ${selectedIds.size} Selected File(s)?`
                    : clearConfirmType === "images"
                    ? "Delete All AI Generated Images?"
              ...
              <div className="flex items-center justify-center gap-2 pt-1">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (clearConfirmType === "batch") handleBatchDelete();
                    else handleClearStorage(clearConfirmType);
                  }}
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs rounded-xl transition cursor-pointer shadow-xs"
                >
                  Delete Permanently
