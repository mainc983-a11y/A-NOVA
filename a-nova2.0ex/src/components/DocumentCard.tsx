...
export const DocumentCard: React.FC<DocumentCardProps> = ({
  ...
  return (
    <div
      className={`my-2.5 w-full max-w-md rounded-2xl border transition-all duration-200 shadow-xs hover:shadow-md relative overflow-hidden p-3 sm:p-3.5 select-none ${
        isDark
          ? "bg-zinc-900/90 border-zinc-800 text-zinc-100 hover:border-zinc-700/80"
    ...
    >
      ...
      <div className="flex items-center gap-3">
        {/* File Type Icon */}
        <div
          className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center shrink-0 border ${cfg.bg} ${cfg.border} ${cfg.color}`}
        >
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-xs sm:text-sm font-semibold truncate text-zinc-900 dark:text-zinc-100 tracking-tight">
            {fileName}
          </h4>
      ...
      {/* Compact Action Bar */}
      <div className="mt-2.5 pt-2 border-t border-zinc-100 dark:border-zinc-800/80 flex items-center gap-1 sm:gap-2 text-xs font-sans">
        <button
          type="button"
          onClick={handleDownload}
          disabled={downloading}
          className={`flex items-center gap-1.5 py-1.5 px-2.5 rounded-xl transition font-medium text-[11px] sm:text-xs cursor-pointer ${
            downloadSuccess
              ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-semibold"
              : "hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:text-emerald-500 dark:hover:text-emerald-400"
          }`}
          title="Download File"
        >
          {downloadSuccess ? (
        ...
        <button
          type="button"
          onClick={handleShare}
          className="flex items-center gap-1.5 py-1.5 px-2.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:text-amber-500 transition font-medium text-[11px] sm:text-xs cursor-pointer"
          title="Share File"
        >
        ...
        {onEdit && (
          <button
            type="button"
            onClick={handleEdit}
            className="flex items-center gap-1.5 py-1.5 px-2.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:text-blue-500 transition font-medium text-[11px] sm:text-xs cursor-pointer"
            title="Edit File"
          >
        ...
        {onDelete && (
          <button
            ...
            className="flex items-center gap-1.5 py-1.5 px-2.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-950/40 text-zinc-400 hover:text-red-500 transition font-medium text-[11px] sm:text-xs cursor-pointer ml-auto"
            title="Delete File"
          >
