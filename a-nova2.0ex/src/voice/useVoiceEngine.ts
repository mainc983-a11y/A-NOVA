...
export function useVoiceEngine({ onSendMessage, onTranscriptChange, requestPermission }: UseVoiceEngineProps = {}) {
  ...
  const stopVoiceSession = useCallback(async () => {
    ...
    // If native WebSpeech didn't return text (e.g., on mobile or restricted environments),
    // transcribe recorded microphone audio blob directly via backend transcription route!
    if (!textToSet && audioBlob && audioBlob.size > 50) {
      try {
        ...
        console.warn("[useVoiceEngine] Audio transcription error:", err);
      }
    }
  ...
  const submitTranscript = useCallback(
    async (rawText?: string) => {
      ...
      if (!textToSubmit && audioBlob && audioBlob.size > 50) {
        try {
          const geminiTranscript = await sttRef.current.transcribeBlob(audioBlob);
          if (geminiTranscript) {
