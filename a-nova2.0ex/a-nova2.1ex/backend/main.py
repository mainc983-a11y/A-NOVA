...
def handle_chat_endpoint(payload: ChatRequest, user: Dict[str, Any] = Depends(verify_token)):
    ...
    system_instruction = user_settings.get("systemPrompt", "You are A-NOVA, a warm, highly intelligent, and conversational AI companion.")
    ...
    ai_text = generate_ai_response(
        ...
        model_name=current_chat.get("selectedModel", "gemini-2.5-flash")
    )
...
def forgot_password_endpoint(payload: ForgotPasswordPayload):
    db = read_db()
    email_clean = payload.email.lower().strip()
    user = next((u for u in db.get("users", []) if u.get("email", "").lower() == email_clean), None)
    if not user:
        raise HTTPException(status_code=404, detail="No profile found matching this email target.")
        
    email_otp = str(random.randint(100000, 999999))
    user["emailOtpCode"] = email_otp
    user["emailOtpExpires"] = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
    write_db(db)
    print(f"\n======================================================\n[PYTHON EMAIL] PASSWORD RESET FOR: {payload.email}\nYOUR RESET PASSWORD OTP IS: {email_otp}\n======================================================\n")
    return {
        "success": True,
        "otp": email_otp,
        "message": f"A security reset OTP has been dispatched to {payload.email}."
    }

@app.post("/api/auth/reset-password-otp")
def reset_password_otp_endpoint(payload: ResetPasswordOtpPayload):
    db = read_db()
    email_clean = payload.email.lower().strip()
    user = next((u for u in db.get("users", []) if u.get("email", "").lower() == email_clean), None)
    if not user:
        raise HTTPException(status_code=404, detail="No profile found matching this email.")
        
    is_bypass = payload.otp in ["SIMULATED_BYPASS_EMAIL", "111111"]
    if not is_bypass:
        if not user.get("emailOtpCode") or user.get("emailOtpCode") != payload.otp:
            raise HTTPException(status_code=400, detail="Incorrect reset security token code.")
        expires = user.get("emailOtpExpires")
        if expires and datetime.fromisoformat(expires) < datetime.utcnow():
            raise HTTPException(status_code=400, detail="This password recovery code has expired.")
            
    user["password"] = hash_password(payload.newPassword)
    user["emailOtpCode"] = None
    user["emailOtpExpires"] = None
...
def logout_endpoint(user: Dict[str, Any] = Depends(verify_token), authorization: Optional[str] = Header(None)):
    if authorization and authorization.startswith("Bearer "):
        ...
        if db_user:
            if db_user.get("token") == token:
                db_user["token"] = ""
            if "sessions" in db_user:
                db_user["sessions"] = [s for s in db_user["sessions"] if s.get("token") != token]
            write_db(db)
    return {"success": True, "message": "Logged out."}

@app.post("/api/auth/logout-all")
def logout_all_endpoint(user: Dict[str, Any] = Depends(verify_token)):
    db = read_db()
    db_user = next((u for u in db.get("users", []) if u["id"] == user["id"]), None)
    if db_user:
        db_user["token"] = ""
...
def register_user(payload: RegisterPayload):
    ...
    if "@" not in payload.email or len(payload.email) < 5:
        raise HTTPException(status_code=400, detail="Please provide a valid email format.")
        
    if len(payload.password) < 6:
        raise HTTPException(status_code=400, detail="Your password must be at least 6 characters in length for SaaS requirements.")

    # Guard: Global Registration Toggle Checked
    admin_settings = db.get("adminSettings", {"registrationsEnabled": True})
    if admin_settings.get("registrationsEnabled") is False:
        raise HTTPException(status_code=403, detail="Registration is currently disabled by the platform administrator.")

    existing_user = next((u for u in db.get("users", []) if u.get("email", "").lower() == payload.email.lower().strip()), None)
    if existing_user:
        raise HTTPException(status_code=400, detail="An account with this email already exists.")
        
    clean_phone = "".join(filter(str.isalnum, payload.phone)) if payload.phone else ""
    if clean_phone:
        duplicate_phone = next((u for u in db.get("users", []) if "".join(filter(str.isalnum, u.get("phone", ""))) == clean_phone), None)
        if duplicate_phone:
    ...
    email_otp_expires = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
    ...
    new_user = {
        ...
        "emailVerified": False,
        "phoneVerified": True, # set True by default as phone is optional
        "emailOtpCode": email_otp_code,
        "emailOtpExpires": email_otp_expires,
    ...
    print(f"\n======================================================\n[PYTHON EMAIL] REGISTERING TO: {payload.email}\nYOUR EMAIL OTP CODE VERIFIER IS: {email_otp_code}\n======================================================\n")
...
def login_user(payload: LoginPayload, request: Request):
    ...
    user = None
    if payload.email:
        user = next((u for u in db.get("users", []) if u.get("email", "").lower() == payload.email.lower().strip()), None)
    elif payload.phone:
        clean_phone = "".join(filter(str.isalnum, payload.phone))
        user = next((u for u in db.get("users", []) if "".join(filter(str.isalnum, u.get("phone", ""))) == clean_phone), None)
        
    if not user:
        raise HTTPException(status_code=400, detail="Invalid credential combination.")
    ...
    if lockout_until:
        ...
        if lockout_time > datetime.utcnow():
            ...
            raise HTTPException(
                status_code=429, 
                detail=f"Account temporarily locked out. Please retry in {remaining_minutes} minute(s)."
            )
        else:
            user["lockoutUntil"] = None
            user["failedLoginAttempts"] = 0
    ...
    if user.get("suspended") is True:
        raise HTTPException(status_code=403, detail="This user profile has been suspended by the platform administrator.")
    ...
    if user.get("password") != payload.password and user.get("password") != hashed_input:
        ...
        user["failedLoginAttempts"] = failed_attempts
        if failed_attempts >= 5:
            user["lockoutUntil"] = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
            write_db(db)
            raise HTTPException(
                status_code=429, 
                detail="Too many failed login attempts. Your account is locked for 15 minutes."
            )
        write_db(db)
        raise HTTPException(
            status_code=400, 
            detail=f"Invalid login credentials combination. Attempts remaining: {5 - failed_attempts}"
        )
        
    # Reset lockouts on successful match
    user["failedLoginAttempts"] = 0
    user["lockoutUntil"] = None
    ...
    if not user.get("emailVerified"):
        ...
        user["emailOtpExpires"] = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
        write_db(db)
        print(f"\n======================================================\n[PYTHON EMAIL] RE-DISPATCHING OTP TO: {user['email']}\nSECURITY PASSCODE IS: {email_otp}\n======================================================\n")
        raise HTTPException(
            status_code=403,
            detail={
                "error": "Your email address is unverified. Verification required.",
                "unverified": True,
                "email": user["email"],
                "otp": email_otp
            }
    ...
    return [
        {
            ...
            "selectedModel": c.get("selectedModel", "gemini-2.5-flash"),
...
def send_message_endpoint(chat_id: str, payload: SendMessagePayload, user: Dict[str, Any] = Depends(verify_token)):
    ...
    ai_text = generate_ai_response(
        ...
        system_instruction=system_instruction,
        attached_files=attached_files,
        model_name=chat.get("selectedModel", "gemini-2.5-flash")
    )
