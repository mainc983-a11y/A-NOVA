...
// Atomic file write helper to prevent zero-byte reads / locks during concurrent IDE or server reads
function safeWriteFileSync(filePath: string, content: string) {
  const dir = path.dirname(filePath);
  if (!fs.existsSync(dir)) {
    try { fs.mkdirSync(dir, { recursive: true }); } catch (e) {}
  }
  const tmpPath = path.join(dir, `.tmp_${path.basename(filePath)}_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`);
  try {
...
function detectUserIntent(promptText: string) {
  ...
  const explicitDocumentRegex = /\b(create|generate|make|build|export|convert|download|save as|output as)\s+(a|an|me|us|the|this)?\s*(pdf|docx|word|pptx|ppt|powerpoint|excel|xlsx|csv|txt)\b/i;
  const isExplicitDocument = explicitDocumentRegex.test(text) && !/\b(image|picture|photo|artwork|drawing|sketch|wallpaper|logo|icon|illustration)\s+(only|file|instead)\b/i.test(text);

  // Image keywords & action verbs
  const directImageVerbRegex = /\b(draw|paint|sketch|illustrate|render)\b/i;
  const imageKeywordsRegex = /\b(image|images|picture|pictures|photo|photos|photograph|photographs|artwork|artworks|drawing|drawings|sketch|sketches|painting|paintings|wallpaper|wallpapers|logo|logos|icon|icons|illustration|illustrations|render|renders|portrait|portraits|landscape|avatar|avatars|graphic|graphics|diagram|diagrams)\b/i;
  const creationVerbRegex = /\b(generate|create|make|produce|design|edit|build)\b/i;
