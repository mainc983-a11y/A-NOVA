...
async function checkSupabaseTableAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const timeoutPromise = new Promise<{ error: any }>((resolve) =>
      setTimeout(() => resolve({ error: new Error("Supabase network request timed out (2.5s)") }), 2500)
    );

    const res: any = await Promise.race([checkPromise, timeoutPromise]);

    if (!res || res.error) {
      const errDetail = res?.error?.message || res?.error || "Unknown response error";
      console.warn(`[SUPABASE DIAGNOSIS] Table: 'user_settings' | Query: '.select("user_id").limit(1)' | Error: ${errDetail}`);
      isSupabaseTableAvailable = false;
      return false;
    }
    ...
    console.warn(`[SUPABASE DIAGNOSIS] Table: 'user_settings' | Query: '.select("user_id").limit(1)' | Exception: ${err?.message || err}`);
    isSupabaseTableAvailable = false;
    return false;
  }
}
...
function getOrCreateSubscription(userId: string, userPlanStatus?: string, isGuestUser?: boolean) {
  ...
  let initialPlanId = "none";
  let initialStatus = "none";
  let planName = "No Active Subscription";

  if (!isGuest && userPlanStatus && userPlanStatus !== "none" && !userPlanStatus.toLowerCase().includes("none")) {
  ...
  const newSub = {
    ...
    paymentMethods: [
      {
        ...
        type: "UPI",
        provider: "PhonePe",
        details: isGuest ? "guest@upi" : "user@okaxis",
        isDefault: true,
        expiry: "N/A"
      }
    ],
    billingHistory: initialStatus === "active" ? [
      {
        ...
        planName: planName,
        amountINR: initialPlanId === "pro" ? 499 : initialPlanId === "standard" ? 199 : 99,
        status: "Paid",
    ...
    usage: {
      messages: { current: 0, max: initialPlanId === "none" ? 20 : 1000 },
      images: { current: 0, max: initialPlanId === "none" ? 0 : 50 },
      voiceMinutes: { current: 0, max: initialPlanId === "none" ? 0 : 300 },
      fileUploads: { current: 0, max: initialPlanId === "none" ? 2 : 100 },
      storageGb: { current: 0.05, max: initialPlanId === "none" ? 0.1 : 10 }
    }
  };
