...
export default function ProfilePictureSection({
  ...
  return (
    <div className="p-5 md:p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800/80 shadow-xs space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-center justify-between gap-4 pt-1">
        {/* Avatar Display */}
        <div className="flex items-center gap-4 min-w-0">
          <div className="relative shrink-0">
            <div className="w-20 h-20 sm:w-22 sm:h-22 rounded-full p-0.5 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700/80 shadow-md flex items-center justify-center overflow-hidden">
              <UserAvatar 
                src={currentPhotoUrl} 
          ...
          <div className="min-w-0">
            <h4 className="text-sm font-semibold text-zinc-900 dark:text-zinc-200 truncate">{displayName || username}</h4>
            {username && !username.toLowerCase().startsWith("guest") && username.toLowerCase() !== "user" && (
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-mono truncate">@{username}</p>
            )}
            <p className="text-[11px] text-zinc-500 mt-1">
              {currentPhotoUrl ? "Custom profile picture uploaded" : "Default letter avatar"}
            </p>
          </div>
        </div>

        {/* 3 Explicit Action Buttons: View, Change, Remove */}
        <div className="flex items-center gap-2 flex-wrap shrink-0 w-full sm:w-auto justify-end">
          {/* View Button */}
          <button
            type="button"
            onClick={() => setShowViewModal(true)}
            className="px-3.5 py-1.5 bg-zinc-100 dark:bg-zinc-800/90 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 text-xs font-semibold rounded-xl flex items-center gap-1.5 border border-zinc-200 dark:border-zinc-700/60 transition-all cursor-pointer active:scale-95"
          ...
          <div className="relative">
            <button
              ...
              className="px-3.5 py-1.5 bg-sky-500 hover:bg-sky-400 text-zinc-950 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-95"
            >
              ...
              {showOptions && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl z-50 p-1.5 space-y-1 text-xs"
                  >
                    <button
                      type="button"
          ...
          {currentPhotoUrl && (
            <button
              ...
              className="px-3.5 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/30 text-xs font-semibold rounded-xl flex items-center gap-1.5 transition-all cursor-pointer active:scale-95"
