...
export default function LoginRegister({ onAuthSuccess }: LoginRegisterProps) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("A verification email has been sent. Please confirm your subscription details via the received link in your email client, then sign in.");
      setPassword("");
      setConfirmPassword("");
    } catch (err: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || "Failed to trigger password recovery.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-xl flex flex-col items-center z-10">
          ...
          <div className="p-8">
                  ...
                  <div className="flex-1">
                    ...
                    {errorMessage.toLowerCase().includes("invalid login credentials") && (
                      <p className="mt-2 text-zinc-400 text-[10px] leading-relaxed border-t border-red-900/30 pt-1.5">
                        <strong className="text-zinc-200">Note:</strong> If you recently updated your connection, old accounts will not work. Please select the <span className="font-semibold text-purple-400 hover:underline cursor-pointer" onClick={() => handleTabChange("signup")}>"Create Account"</span> tab to sign up a new user.
                      </p>
                    )}
                  </div>
                </motion.div>
            ...
            <form onSubmit={activeTab === "login" ? handleLogin : handleSignup} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-zinc-950/70 border border-zinc-800/90 rounded-2xl text-xs text-white placeholder-zinc-650 focus:border-cyan-500/80 focus:ring-1 focus:ring-cyan-500/30 focus:outline-none transition-all font-sans duration-200"
                  />
                </div>
              </div>
                  ...
                  <label className="text-[10px] text-zinc-400 font-mono uppercase tracking-widest font-medium">
                   ...
                   {activeTab === "login" && (
                    <button 
                      ...
                      className="text-[10px] text-cyan-400/80 hover:text-cyan-400 font-mono transition-colors cursor-pointer"
                    >
                      Forgot?
                    </button>
                  )}
              ...
              <button
                ...
                className={`w-full py-4 px-5 mt-6 bg-gradient-to-r ${
                  activeTab === "login" 
                    ? "from-cyan-500 via-blue-600 to-indigo-650 shadow-[0_0_20px_rgba(6,182,212,0.25)] hover:shadow-[0_0_25px_rgba(6,182,212,0.4)]" 
            ...
            <div className="relative my-7">
              ...
              <div className="relative flex justify-center text-[10px] uppercase font-mono tracking-widest">
                <span className="px-4 bg-zinc-900/10 backdrop-blur text-zinc-550">Or continue with</span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 py-3 px-5 bg-zinc-950/80 hover:bg-zinc-900 border border-zinc-800 rounded-2xl text-xs font-semibold hover:border-zinc-700 transition duration-200 select-none cursor-pointer text-zinc-350"
            >
        ...
        >
          <p className="text-[10px] text-zinc-550 font-mono tracking-wide flex items-center justify-center gap-1.5">
            ...
            Node ID: <span className="text-zinc-400 font-bold select-text">a-nova-node-2026-prod</span>
          </p>
