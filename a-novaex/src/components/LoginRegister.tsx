...
export default function LoginRegister({ onAuthSuccess }: LoginRegisterProps) {
  ...
  const [email, setEmail] = useState(() => {
    try {
      return localStorage.getItem("a_nova_remembered_email") || "";
    } catch {
      return "";
    }
  });
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      }, "LoginRegister.tsx:handleGuestLogin");
      ...
      if (res.ok && resData.user) {
        ...
        const cleanUser = {
          ...resData.user,
          email: "",
          username: resData.user.username === "guest_user" ? "Guest" : (resData.user.username || "Guest"),
          displayName: resData.user.displayName === "guest_user" ? "Guest User" : (resData.user.displayName || "Guest User")
        };
        localStorage.setItem("a_nova_auth_token", resData.token);
        localStorage.setItem("a_nova_user_data", JSON.stringify(cleanUser));
        onAuthSuccess(resData.token, cleanUser);
      } else {
  ...
  return (
    <div 
      ...
      className="min-h-screen w-full bg-white dark:bg-zinc-950 flex flex-col items-center justify-center p-4 relative overflow-y-auto font-sans text-zinc-900 dark:text-white md:p-8 select-none transition-colors"
    >
      {/* Background radial atmosphere */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(14,165,233,0.08)_0%,rgba(244,244,245,0.98)_70%)] dark:bg-[radial-gradient(ellipse_at_top,rgba(14,165,233,0.12)_0%,rgba(9,9,11,0.98)_70%)] pointer-events-none z-0" />
      ...
      <div className="w-full max-w-md flex flex-col items-center z-10 my-auto py-4 sm:py-6 md:py-8">
        ...
        <motion.div 
          ...
          className="flex flex-col items-center mb-4 sm:mb-6 text-center"
        >
          <AnovaLogo size="lg" showText={true} subtitle="INTELLIGENCE WORKSPACE" animated={true} />
        </motion.div>
        ...
        <motion.div 
          ...
          className="w-full bg-white/90 dark:bg-zinc-900/80 backdrop-blur-2xl border border-zinc-200/90 dark:border-zinc-800/90 rounded-3xl shadow-xl dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] overflow-hidden"
        >
          {/* Subtle top accent gradient */}
          <div className="h-1 bg-gradient-to-r from-cyan-400 via-indigo-500 to-purple-500 w-full" />

          {/* Tab Selection */}
          <div className="flex border-b border-zinc-200 dark:border-zinc-800/80 p-1.5 bg-zinc-100/80 dark:bg-zinc-950/60 m-3 rounded-2xl">
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-semibold tracking-wide rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${
                activeTab === "login" 
                  ? "bg-white dark:bg-zinc-800 text-cyan-600 dark:text-cyan-400 border border-zinc-200 dark:border-zinc-700/60 shadow-sm" 
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-xs font-semibold tracking-wide rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${
                activeTab === "signup" 
                  ? "bg-white dark:bg-zinc-800 text-purple-600 dark:text-purple-400 border border-zinc-200 dark:border-zinc-700/60 shadow-sm" 
                  : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200"
              }`}
            >
          ...
          <div className="px-6 pb-6 pt-2">
            ...
            <AnimatePresence mode="wait">
              {errorMessage && (
                <motion.div 
                  ...
                  className="p-3.5 mb-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/50 rounded-2xl text-red-600 dark:text-red-400 text-xs flex items-start gap-2.5"
                >
              ...
              {successMessage && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, height: "auto", y: 0 }}
                  exit={{ opacity: 0, height: 0, y: -10 }}
                  className="p-3.5 mb-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl text-emerald-600 dark:text-emerald-400 text-xs flex items-start gap-2.5"
                >
                  <CheckCircle className="w-4 h-4 shrink-0 text-emerald-500 dark:text-emerald-400 mt-0.5" />
                  <div className="flex-1 min-w-0">
            ...
            <form onSubmit={activeTab === "login" ? handleLogin : handleSignup} className="space-y-4">
              {/* Email Input */}
              <div>
                <label className="block text-[10px] text-zinc-500 dark:text-zinc-400 font-mono uppercase tracking-wider mb-1.5 font-semibold">
                  Email Address
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <Mail className="h-4 w-4 text-zinc-400 dark:text-zinc-500 group-focus-within:text-cyan-500 dark:group-focus-within:text-cyan-400 transition-colors" />
                  </div>
                  <input
                    type="email"
                    required
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800 rounded-2xl text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 focus:outline-none transition-all font-sans"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <label className="text-[10px] text-zinc-500 dark:text-zinc-400 font-mono uppercase tracking-wider font-semibold">
                    Password
                  </label>
                  {activeTab === "login" && (
                    <button 
                      ...
                      className="text-[10px] text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 font-mono transition-colors cursor-pointer"
                    >
                      Forgot password?
                    </button>
                  )}
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <Lock className="h-4 w-4 text-zinc-400 dark:text-zinc-500 group-focus-within:text-purple-500 dark:group-focus-within:text-purple-400 transition-colors" />
                  </div>
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800 rounded-2xl text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-all"
                  />
                  <button
                    type="button"
              ...
              <button
                ...
                className={`w-full py-3 px-4 mt-2 bg-gradient-to-r ${
                  activeTab === "login" 
                    ? "from-cyan-500 via-indigo-600 to-purple-600 hover:from-cyan-400 hover:to-purple-500 shadow-md shadow-cyan-950/20 dark:shadow-cyan-950/50" 
                    : "from-purple-600 via-indigo-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 shadow-md shadow-purple-950/20 dark:shadow-purple-950/50"
            ...
            <div className="relative my-5">
              <div className="absolute inset-y-1/2 left-0 right-0 h-[1px] bg-zinc-200 dark:bg-zinc-800/80" />
              <div className="relative flex justify-center text-[9px] uppercase font-mono tracking-widest">
                <span className="px-3 bg-white dark:bg-zinc-900 text-zinc-500">Or choose instant access</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-2.5 px-3 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-950/70 dark:hover:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:border-zinc-300 dark:hover:border-zinc-700 transition duration-150 cursor-pointer"
              >
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-2.5 px-3 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-950/70 dark:hover:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:border-zinc-300 dark:hover:border-zinc-700 hover:text-cyan-600 dark:hover:text-cyan-400 transition duration-150 cursor-pointer"
              >
