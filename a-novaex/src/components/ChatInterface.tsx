...
const CodeBlock = memo(function CodeBlock({
  ...
  const isCodeDark = isDark;
  return (
    <div className={`my-4 rounded-xl border overflow-hidden font-mono shadow-sm ${
      isCodeDark ? "border-zinc-805 bg-zinc-950" : "border-zinc-200 bg-zinc-100"
    }`}>
      <div className={`flex items-center justify-between px-4 py-2 border-b text-[10px] font-mono select-none ${
        isCodeDark ? "bg-zinc-900 border-zinc-800 text-zinc-400" : "bg-zinc-150 border-zinc-200 text-zinc-500"
      }`}>
      ...
      <pre className="p-3 sm:p-4 overflow-x-auto text-[11px] sm:text-[12px] leading-relaxed font-mono max-h-96 select-text max-w-full">
        <code className={isCodeDark ? "text-zinc-250" : "text-zinc-800"}>{codeText}</code>
      </pre>
    </div>
  );
});
      ...
      <div key={`${messageId}_text_${index}`} className="space-y-1.5 font-sans break-words max-w-full overflow-hidden">
        {lines.map((line, lIdx) => {
          ...
          if (markdownImgMatch) {
            ...
            return (
              <div key={lIdx} className="my-3 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-950/40 p-1.5 shadow-md max-w-lg">
                <img 
                  src={imageUrl} 
                  alt={altText} 
                ...
                />
                <div className="p-2 flex items-center justify-between text-[11px] text-zinc-400 font-sans">
                  <span className="truncate max-w-[250px] font-medium text-zinc-300">{altText}</span>
                  <a 
                    href={imageUrl} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    download="generated_artwork.png" 
                    className="text-sky-400 hover:text-sky-300 hover:underline flex items-center gap-1 font-mono text-[10px]"
                  >
                    Download ↗
                  </a>
                </div>
          ...
          <div className="mt-0.5 max-w-full overflow-hidden break-words w-full">
            {isAssistant ? (
              <div className="text-left select-text max-w-full overflow-hidden break-words">
                ...
                {parsedDoc && (
                  <SimpleDocumentDownload document={parsedDoc} isDark={isDark} />
                )}
              </div>
            ) : (
...
const ChatInterface = React.memo(function ChatInterface({
  ...
  useEffect(() => {
    if (textareaInputRef.current) {
      ...
      textareaInputRef.current.style.height = `${Math.min(scrollH, 160)}px`;
    }
  }, [inputText]);
  ...
  return (
    ...
    >
      ...
      >
        {!session || !session.messages || session.messages.length === 0 ? (
          ...
          <div className="w-full max-w-full min-w-0 py-0.5 md:py-2">
            ...
            {/* Typing indicator directly on chat background */}
            {loading && (
      ...
      <footer className="p-1.5 sm:p-2.5 md:p-3 pb-2.5 sm:pb-3 pb-safe shrink-0 w-full max-w-full min-w-0 relative">
        <div className="w-full max-w-2xl sm:max-w-3xl px-2 sm:px-3 mx-auto space-y-1.5 sm:space-y-2 relative">
          ...
          >
            ...
            {webSearchActive && (
              <div className="flex items-center gap-1.5 px-3 pt-1.5 select-none text-emerald-500 text-[10px] font-bold font-mono">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 px-1.5 py-1 sm:px-2 sm:py-1 md:px-2.5 md:py-1">
              ...
              <div className="flex items-center justify-center shrink-0">
                <button
                  ...
                  className={`relative flex h-7 w-7 sm:h-7.5 sm:w-7.5 md:h-8 md:w-8 items-center justify-center rounded-full transition-all cursor-pointer before:absolute before:-inset-2 sm:before:hidden before:content-[''] ${
                    isPlusMenuOpen 
                  ...
                  title="Attach & Tools"
                >
                  <Plus className={`w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 transition-transform duration-200 ${isPlusMenuOpen ? "rotate-45" : ""}`} />
                </button>
              </div>
              ...
              <textarea
                ...
                className="no-focus-glow w-full bg-transparent px-1 sm:px-1.5 py-0.5 text-[13.5px] sm:text-[14px] md:text-[14px] leading-snug focus:outline-none focus:ring-0 focus:shadow-none placeholder-zinc-400 dark:placeholder-zinc-500 font-sans resize-none block flex-1 max-h-32 min-h-[1.25rem] sm:min-h-[1.375rem] overflow-y-auto transition-[height] duration-100 ease-out my-auto caret-cyan-500 dark:caret-cyan-400"
              />

              {/* Action Buttons Group (Voice Mic / Send + Voice Assistant) */}
              <div className="flex items-center gap-1 sm:gap-1 shrink-0">
                ...
                {(!inputText.trim() && attachedFiles.length === 0) ? (
                  ...
                  >
                    {isListening ? <MicOff className="w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" /> : <Mic className="w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" />}
                  </button>
                ) : (
                  <button
                    type="submit"
                    disabled={loading || (!inputText.trim() && attachedFiles.length === 0)}
                    className="relative flex h-7 w-7 sm:h-7.5 sm:w-7.5 md:h-8 md:w-8 items-center justify-center rounded-full bg-purple-600 hover:bg-purple-550 text-white shadow-xs transition-all cursor-pointer disabled:opacity-50 shrink-0 before:absolute before:-inset-2 sm:before:hidden before:content-['']"
                    title="Send message"
                  >
                    {loading ? <Loader2 className="w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 animate-spin" /> : <ArrowUp className="w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 stroke-[2.5]" />}
                  </button>
                )}
                ...
                >
                  <AudioLines className="w-3.5 h-3.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" />
                </button>
              </div>
            </div>
