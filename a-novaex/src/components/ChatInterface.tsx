...
import VoiceVolumeVisualizer from "./VoiceVolumeVisualizer";
...
const MessageBubble = memo(function MessageBubble({
  ...
  const shareLabel = React.useMemo(() => {
    const hasImage = (msg.generatedImages && msg.generatedImages.length > 0) ||
      (msg.content && (msg.content.includes("![") || msg.content.includes("data:image/")));
          ...
          <div className="mt-0.5 max-w-full overflow-hidden break-words w-full">
            {isAssistant ? (
              <div className="text-left select-text max-w-full overflow-hidden break-words">
                {isEditingResponse ? (
                  <div className="w-full flex flex-col gap-2 my-2 p-3 rounded-2xl border bg-zinc-900/90 border-zinc-700/80">
                    <textarea
                      value={messageText}
                      onChange={(e) => setMessageText(e.target.value)}
                      className={`w-full p-3 text-xs sm:text-sm rounded-xl border focus:outline-hidden focus:ring-2 focus:ring-sky-500/50 resize-y min-h-[120px] font-sans ${
                        isDark ? "bg-zinc-950 border-zinc-800 text-zinc-100" : "bg-white border-zinc-200 text-zinc-900"
                      }`}
                      placeholder="Edit AI response content..."
                    />
                    <div className="flex items-center gap-2 justify-end">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium rounded-lg text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setIsEditingResponse(false);
                          setToastText("Response edited");
                        }}
                        className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-sky-500 text-white hover:bg-sky-600 transition-colors cursor-pointer shadow-sm"
                      >
                        Save
                      </button>
                    </div>
...
const ChatInterface = React.memo(function ChatInterface({
  ...
  const [voiceAssistantState, setVoiceAssistantState] = useState<'listening' | 'thinking' | 'speaking' | 'idle' | 'error' | 'requesting'>('idle');
  ...
  const voiceAssistantStateRef = useRef<'listening' | 'thinking' | 'speaking' | 'idle' | 'error' | 'requesting'>('idle');
  ...
  const openVoiceAssistantMode = () => {
    ...
    setVoiceAssistantState('requesting');
          ...
          >
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 px-3.5 py-1 sm:px-4 sm:py-2">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 my-auto">
                {/* Real-time compact voice visualizer when dictating */}
                {isListening && (
        ...
        <AnimatePresence>
          {isVoiceAssistantOpen && (
            ...
            >
              ...
              {/* Main Center Area: Voice Visualizer */}
              <div className="flex-1 w-full max-w-md flex flex-col items-center justify-center gap-4 my-auto text-center px-4">
                <VoiceVolumeVisualizer
                  state={voiceAssistantState}
                  isActive={isVoiceAssistantOpen && voiceAssistantState === "listening"}
                  transcript={voiceTranscript}
                  aiResponseText={voiceAiResponseText}
                  errorMessage={voiceErrorMsg || undefined}
                  onOrbClick={() => {
                    if (voiceAssistantState === 'speaking') {
                      handleInterruptVoiceAssistant();
                    } else if (voiceAssistantState === 'listening' || voiceAssistantState === 'idle') {
                      startVoiceAssistantListening();
                    }
                  }}
                  variant="full"
