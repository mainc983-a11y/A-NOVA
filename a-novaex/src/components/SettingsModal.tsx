...
export default function SettingsModal({
  ...
  const hasUnsavedFormChanges = useMemo(() => {
    ...
    return customInstructionsChanged || passwordFormActive;
  }, [settings, aboutMe, respondWay, systemPrompt, currentPassword, newPassword, confirmNewPassword]);

  // Immediate Auto-Save Handler for Simple Toggle & Select Settings
  const handleAutoSaveSetting = async (updates: Partial<Settings>) => {
    const isDark = (updates.theme || theme) === 'dark' || ((updates.theme || theme) === 'system' ? true : false);
    const updated: Settings = {
      ...
      aboutMe: updates.aboutMe ?? aboutMe,
      respondWay: updates.respondWay ?? respondWay,
...
===================================================================`;
  ...
  const NAV_ITEMS: { id: TabCategory; label: string; icon: any; badge?: string }[] = [
    ...
    { id: "subscription", label: "Subscription / Plan", icon: CreditCard, badge: planStatus === "none" || !planStatus ? "No Plan" : planStatus },
  ...
  const ToggleSwitch = ({ checked, onChange, label, description }: { checked: boolean; onChange: (v: boolean) => void; label: string; description?: string }) => (
    <div className="flex items-center justify-between p-3.5 sm:p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800/80 hover:border-zinc-300 dark:hover:border-zinc-700/60 transition-all min-h-[52px]">
      <div className="pr-3 min-w-0 flex-1">
        ...
        {description && <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 leading-snug">{description}</p>}
      </div>
      <button
        type="button"
        role="switch"
  ...
  return (
    <div id="settings_modal_wrapper" className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4 lg:p-6 overflow-hidden select-none">
      ...
      <motion.div
        ...
        className="w-full h-full h-[100dvh] w-[100vw] max-w-full sm:max-w-3xl md:max-w-4xl lg:max-w-5xl sm:h-auto sm:max-h-[88vh] bg-white dark:bg-zinc-950 border-0 sm:border border-zinc-200 dark:border-zinc-800/90 rounded-none sm:rounded-3xl shadow-2xl relative z-10 flex flex-col overflow-hidden font-sans text-zinc-900 dark:text-zinc-100"
      >
        {/* UNIFIED SETTINGS INDEX PAGE (All Devices: Mobile, Tablet, Desktop) */}
        <div className={`flex flex-col h-full h-[100dvh] sm:h-auto w-full bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white overflow-hidden ${mobileView === "menu" ? "flex" : "hidden"}`}>
          {/* Fixed Header */}
          <div className="flex items-center justify-between px-4 sm:px-6 md:px-8 py-3.5 sm:py-4 border-b border-zinc-200 dark:border-zinc-800/80 sticky top-0 bg-white/95 dark:bg-zinc-950/95 z-30 backdrop-blur-md pt-[max(14px,env(safe-area-inset-top))]">
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Name, email, avatar & password</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Active Plan: {planStatus}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-full">
                      Active
                    </span>
                    <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                  </div>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Theme ({theme}), accent color, density</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Model: {defaultModel.replace('gemini-', '')}</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">About me & response style preferences</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">TTS Profile: {voiceName} ({speechSpeed}x)</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Alerts, sound effects, security</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">API keys, active sessions, 2FA</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">Usage: {chatStats.formattedStorage}</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] sm:text-xs text-zinc-500 dark:text-zinc-400 truncate">v2.5.0 • Support, shortcuts & feedback</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0" />
                </button>
        ...
        <div className={`flex-1 flex flex-col min-w-0 w-full h-full h-[100dvh] sm:h-auto bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white overflow-hidden ${mobileView === "content" ? "flex" : "hidden"}`}>
          
          {/* Section Header */}
          <div className="px-4 sm:px-6 md:px-8 py-3.5 sm:py-4 border-b border-zinc-200 dark:border-zinc-800/80 flex items-center justify-between bg-white/95 dark:bg-zinc-950/95 sticky top-0 z-30 shrink-0 backdrop-blur-md pt-[max(14px,env(safe-area-inset-top))]">
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === "chat" && (
                ...
                >
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    <h4 className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">Default Model</h4>
                    <select
                      value={defaultModel}
                      onChange={(e) => {
                  ...
                  {/* System Persona */}
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    <h4 className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">System Persona</h4>
                    <textarea
                      rows={3}
                      value={systemPrompt}
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800/80 pb-3">
                      <div>
                        <h4 className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">Custom Instructions</h4>
                        <p className="text-[11px] text-zinc-500 dark:text-zinc-400">Personalize responses to your preferences</p>
                      </div>
                      <button
                        type="button"
                        role="switch"
                    ...
                    {customInstructionsEnabled && (
                      <div className="space-y-3 pt-1">
                        <div>
                          <label className="block text-[11px] font-medium text-zinc-600 dark:text-zinc-400 mb-1">What should AI know about you?</label>
                          <textarea
                            rows={2}
                            value={aboutMe}
                            onChange={(e) => setAboutMe(e.target.value)}
                            placeholder="E.g. Senior software developer working with React..."
                            className="w-full px-3.5 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 focus:border-sky-500/80 text-xs text-zinc-900 dark:text-white rounded-xl outline-none resize-none transition-colors"
                          />
                        </div>
                        <div>
                          <label className="block text-[11px] font-medium text-zinc-600 dark:text-zinc-400 mb-1">How should AI respond?</label>
                          <textarea
                            rows={2}
                            value={respondWay}
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    ...
                    <p className="text-[11px] text-zinc-500 dark:text-zinc-400">Optional custom key override for higher request limits</p>
                    <input
                      type={apiKeyVisible ? "text" : "password"}
                      value={customApiKey}
                      onChange={(e) => {
                        setCustomApiKey(e.target.value);
                        handleAutoSaveSetting({ customApiKey: e.target.value });
                      }}
                      placeholder="AIzaSy..."
                ...
                >
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-4">
                    <ToggleSwitch
                      ...
                      onChange={(val) => {
                        setVoiceEnabled(val);
                        handleAutoSaveSetting({ voiceEnabled: val });
                      }}
                    ...
                    {voiceEnabled && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                        <div>
                          <label className="block text-[11px] font-medium text-zinc-600 dark:text-zinc-400 mb-1.5">Voice Profile</label>
                          <select
                            value={voiceName}
                            onChange={(e) => {
                              setVoiceName(e.target.value);
                              handleAutoSaveSetting({ voiceName: e.target.value as any });
                            }}
                        ...
                        <div>
                          <label className="block text-[11px] font-medium text-zinc-600 dark:text-zinc-400 mb-1.5">Speech Speed ({speechSpeed}x)</label>
                          <input
                            type="range"
                            min="0.5"
                            max="2.0"
                            ...
                            className="w-full h-2 bg-zinc-200 dark:bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-sky-500 mt-2.5"
                          />
                        </div>
                      </div>
                    )}
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-medium text-zinc-600 dark:text-zinc-400 mb-1.5">Dictation Language</label>
                      <select
                        value={voiceLanguage}
                      ...
                      >
                        <span
                          className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full transition-transform ${
                            twoFactorEnabled ? "translate-x-4.5 bg-zinc-950" : "translate-x-0 bg-zinc-400"
                          }`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="space-y-3 pt-1">
                      ...
                      <div className="p-3.5 rounded-xl bg-white dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap mt-2">
                              {["Google Authenticator", "Microsoft Authenticator", "Authy", "1Password"].map((app) => (
                                <span key={app} className="text-[10px] px-2 py-0.5 bg-zinc-800/90 text-zinc-300 border border-zinc-700/60 rounded-md font-medium">
                                  {app}
                                </span>
                              ))}
                            </div>
                        ...
                        <div className="flex items-center sm:self-center shrink-0">
                          <span className="text-[10px] px-2.5 py-1 bg-zinc-800/80 text-zinc-400 border border-zinc-700/50 rounded-lg font-semibold uppercase tracking-wider">
                            Coming Soon
                          </span>
                        </div>
                      </div>
              ...
              {activeTab === "storage" && (
                ...
                >
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-semibold text-zinc-900 dark:text-white uppercase tracking-wider">Storage Usage</h4>
                      <span className="text-xs font-medium text-sky-500 dark:text-sky-400 font-mono">{chatStats.formattedStorage} / 500 MB</span>
                    </div>
                    <div className="w-full h-2.5 bg-zinc-200 dark:bg-zinc-900 rounded-full overflow-hidden border border-zinc-300 dark:border-zinc-800/80">
                      <div 
                  ...
                  <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-3">
                    <h4 className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">Uploaded Attachments</h4>
                    
                    {chatStats.uploadedFilesList.length === 0 ? (
                      <div className="text-center py-8 border border-dashed border-zinc-300 dark:border-zinc-800/80 rounded-2xl">
                        <File className="w-7 h-7 text-zinc-400 dark:text-zinc-600 mx-auto mb-2" />
                        <p className="text-xs font-medium text-zinc-600 dark:text-zinc-400">No file attachments found</p>
                        <p className="text-[11px] text-zinc-500 dark:text-zinc-500 mt-0.5">Files attached in chat sessions will appear here</p>
                      </div>
                    ) : (
                      <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                        {chatStats.uploadedFilesList.map((file) => (
                          <div key={file.id} className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800/80">
                            <div className="flex items-center gap-2.5 min-w-0">
                              <File className="w-4 h-4 text-zinc-600 dark:text-zinc-300 shrink-0" />
                              <div className="min-w-0">
                                <p className="text-xs font-medium text-zinc-900 dark:text-white truncate">{file.name}</p>
                                <p className="text-[10px] text-zinc-500 dark:text-zinc-400">Session: {file.sessionTitle}</p>
                              </div>
                            </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-2.5">
                      <h4 className="text-xs font-semibold text-zinc-900 dark:text-white">Export Account Backup</h4>
                      <p className="text-[11px] text-zinc-600 dark:text-zinc-400">Download a full JSON copy of chats and settings</p>
                      <button
                        type="button"
                        onClick={handleExportData}
                        className="w-full py-2.5 px-3 bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-700/80 text-xs font-medium rounded-xl flex items-center justify-center gap-2 text-zinc-800 dark:text-zinc-200 cursor-pointer transition-colors"
                      >
                    ...
                    <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 border border-zinc-200 dark:border-zinc-800/80 space-y-2.5">
                      <h4 className="text-xs font-semibold text-zinc-900 dark:text-white">Clear History</h4>
                      <p className="text-[11px] text-zinc-600 dark:text-zinc-400">Permanently delete all chat message history</p>
                      {confirmClearHistory ? (
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-300 text-xs font-medium rounded-xl cursor-pointer"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full py-2.5 px-3 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-600 dark:text-rose-400 text-xs font-semibold rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-colors"
                        >
