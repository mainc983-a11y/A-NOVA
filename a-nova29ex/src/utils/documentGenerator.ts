...
export async function exportAndDownloadDocument(doc: GeneratedDocument): Promise<void> {
  if (!doc) {
    throw new Error("Document reference is invalid or empty.");
  }

  const safeDoc: GeneratedDocument = {
    ...doc,
    title: doc.title || "Generated_Document",
  ...
  const filename = sanitizeFilename(safeDoc.title, safeDoc.format);

  switch (safeDoc.format) {
    case "pdf":
      await downloadPDF(safeDoc, filename);
      break;
    case "docx":
      await downloadDOCX(safeDoc, filename);
      break;
    case "pptx":
      await downloadPPTX(safeDoc, filename);
      break;
    case "xlsx":
      downloadXLSX(safeDoc, filename);
      break;
    case "csv":
      downloadCSV(safeDoc, filename);
      break;
    case "md":
      downloadMarkdown(safeDoc, filename);
      break;
    case "txt":
      downloadTXT(safeDoc, filename);
      break;
    case "json":
      downloadJSON(safeDoc, filename);
      break;
    case "html":
      downloadHTML(safeDoc, filename);
      break;
    default:
      await downloadPDF(safeDoc, filename);
      break;
