 ...
 * 1. OpenWeather API: 03b4395efb9ee982a51388ffa09752a6
 * 2. ExchangeRate API: e128e43126c6405572795f11
 * 3. Google Maps Web: AIzaSyAuA8DIY3YBDyCIQyd99C239JqSU2KpfFA
 * 4. Google Maps Android: AIzaSyDukiZ6mXGr6lEx4MB5EWLfi4UuyEuU9ik
 */

export const OPENWEATHER_API_KEY = process.env.OPENWEATHER_API_KEY || "03b4395efb9ee982a51388ffa09752a6";
export const EXCHANGERATE_API_KEY = process.env.EXCHANGERATE_API_KEY || "e128e43126c6405572795f11";
export const GOOGLE_MAPS_WEB_API_KEY = process.env.GOOGLE_MAPS_WEB_API_KEY || "AIzaSyAuA8DIY3YBDyCIQyd99C239JqSU2KpfFA";
export const GOOGLE_MAPS_ANDROID_API_KEY = process.env.GOOGLE_MAPS_ANDROID_API_KEY || "AIzaSyDukiZ6mXGr6lEx4MB5EWLfi4UuyEuU9ik";
...
export async function fetchCurrentWeather(
  ...
  try {
    ...
    if (isMyLocation && coords) {
      url = `https://api.openweathermap.org/data/2.5/weather?lat=${coords.lat}&lon=${coords.lon}&units=${units}&appid=${OPENWEATHER_API_KEY}`;
    } else if (locationQuery && locationQuery.trim()) {
      url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(locationQuery.trim())}&units=${units}&appid=${OPENWEATHER_API_KEY}`;
    } else {
      return null;
    }
...
export async function fetchWeatherForecast(
  ...
  try {
    ...
    if (coords && typeof coords.lat === "number" && typeof coords.lon === "number") {
      url = `https://api.openweathermap.org/data/2.5/forecast?lat=${coords.lat}&lon=${coords.lon}&units=${units}&appid=${OPENWEATHER_API_KEY}`;
    } else if (locationQuery && locationQuery.trim()) {
      url = `https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(locationQuery.trim())}&units=${units}&appid=${OPENWEATHER_API_KEY}`;
    } else {
      return null;
    }
...
export async function convertCurrency(
  ...
  try {
    ...
    const url = `https://v6.exchangerate-api.com/v6/${EXCHANGERATE_API_KEY}/pair/${from}/${to}/${cleanAmount}`;
    const response = await fetch(url);
    if (!response.ok) {
