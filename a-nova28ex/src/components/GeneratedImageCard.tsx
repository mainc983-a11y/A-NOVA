...
const GeneratedImageCard = React.memo(function GeneratedImageCard({
  ...
  const handleDownload = async (e?: React.MouseEvent) => {
    ...
    const urlToDownload = currentUrl || image?.url;
  ...
  const handleImageError = () => {
    if (!fallbackAttempted && image?.prompt) {
      setFallbackAttempted(true);
      const clean = encodeURIComponent(image.prompt.slice(0, 150));
      const seed = Math.floor(Math.random() * 900000) + 100000;
      setCurrentUrl(`https://image.pollinations.ai/prompt/${clean}?width=1024&height=1024&nologo=true&seed=${seed}&model=flux`);
      setIsImgLoaded(false);
    } else {
      setIsImgLoaded(false);
      setHasError(true);
  ...
  const isPending = isGenerating || !image || (!isImgLoaded && !hasError);
  const currentPrompt = image?.prompt || promptText || "Generating image...";

  return (
    <>
      <div
        className={`my-3.5 w-full max-w-full xs:max-w-[340px] sm:max-w-[380px] md:max-w-[370px] mx-auto rounded-2xl sm:rounded-3xl border transition-all duration-300 shadow-md hover:shadow-lg relative overflow-hidden group p-3.5 sm:p-4 ${
          isDark
      ...
      >
        ...
        <div
          ...
          className={`relative w-full aspect-square max-w-[340px] max-h-[340px] mx-auto rounded-xl sm:rounded-2xl overflow-hidden bg-zinc-100 dark:bg-zinc-950/80 border border-zinc-200/80 dark:border-zinc-800/80 flex items-center justify-center transition-all duration-300 ${
          ...
          {(isPending || hasError) && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-5 text-center select-none">
              ...
              {hasError ? (
                ...
                <>
                  <div className="relative z-10 w-11 h-11 rounded-2xl bg-sky-500/10 dark:bg-sky-500/20 flex items-center justify-center mb-1 border border-sky-500/20">
