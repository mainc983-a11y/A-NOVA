...
const supabaseServer = createClient(baseSupabaseUrl, SUPARBASE_PUBLIC_KEY);
...
async function authenticate(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  try {
    ...
    if (error || !supabaseUser) {
      // Look up fallback legacy or bypass token
      const db = readDb();
      const legacyUser = db.users.find((u: any) => u.token === token || (u.sessions && u.sessions.some((s: any) => s.token === token)));
      if (legacyUser) {
        req.body.user = legacyUser;
        return next();
      }
    ...
    // 2. Locate or dynamically provision database user profile record
    let localUser = db.users.find((u: any) => u.id === supabaseUser.id || u.email.toLowerCase() === supabaseUser.email!.toLowerCase());
...
app.post("/api/auth/register", (req, res) => {
  ...
  // Basic email structures validation
  if (!email.includes("@") || email.length < 5) {
    return res.status(400).json({ error: "Please provide a valid email format." });
  }

  if (password.length < 6) {
    return res.status(400).json({ error: "Your password must be at least 6 characters in length for SaaS requirements." });
  }
  ...
  const newUser = {
    ...
    phone: phone || "",
    emailVerified: !REQUIRE_OTP_VERIFICATION, // Auto-verify based on flag
    phoneVerified: true, // phone is optional, so default true to prevent blockade
    emailOtpCode,
    emailOtpExpires,
    ...
    sessions: [
      {
        token,
        userAgent: req.headers["user-agent"] || "Server register initialization",
        ip: req.ip || "127.0.0.1",
  ...
  if (db.pendingOtps?.[(phone || "").replace(/[^0-9+]/g, "")]) {
    delete db.pendingOtps[(phone || "").replace(/[^0-9+]/g, "")];
  }
  ...
  if (REQUIRE_OTP_VERIFICATION) {
    console.log(`\n======================================================\n[EMAIL SATELLITE] REGISTERING TO: ${email}\nYOUR EMAIL OTP CODE VERIFIER IS: ${emailOtpCode}\nEXPIRES IN: 15 minutes\n======================================================\n`);
  }

  res.status(201).json({ 
    token, 
    otp: REQUIRE_OTP_VERIFICATION ? emailOtpCode : undefined, // expose code only if OTP is enabled
    user: { 
      id: newUser.id, 
      email: newUser.email, 
      phone: newUser.phone, 
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (email) {
    user = db.users.find((u: any) => u.email.toLowerCase() === email.toLowerCase().trim());
  } else if (phone) {
    const cleanPhone = phone.replace(/[^0-9+]/g, "");
  ...
  if (user.lockoutUntil) {
    ...
    if (lockoutTime > new Date()) {
      const timeRemaining = Math.ceil((lockoutTime.getTime() - Date.now()) / 1000 / 60);
      return res.status(429).json({ 
        error: `Account is temporarily locked out due to multiple failed login attempts. Please retry in ${timeRemaining} minute(s).` 
      });
    } else {
      // Lockout expired
  ...
  if (user.password !== password && user.password !== hashedInput) {
    ...
    if (user.failedLoginAttempts >= 5) {
      user.lockoutUntil = new Date(Date.now() + 15 * 60 * 1000).toISOString(); // 15 mins block
      writeDb(db);
      return res.status(429).json({ 
    ...
    return res.status(400).json({ 
      error: `Invalid login combination. Attempts left: ${5 - user.failedLoginAttempts}` 
    });
  }
  ...
  // Auto-verify if OTP is not required
  if (!REQUIRE_OTP_VERIFICATION) {
    user.emailVerified = true;
  }

  // STRICT ENFORCEMENT: ONLY Email verification checks block login!
  if (!user.emailVerified) {
    // Generate new OTP during email verification failure to allow immediate flow
    const emailOtpCode = Math.floor(100000 + Math.random() * 900000).toString();
    const emailOtpExpires = new Date(Date.now() + 15 * 60 * 1050).toISOString();
    ...
    console.log(`\n======================================================\n[EMAIL SATELLITE] RE-DISPATCHING OTP TO: ${user.email}\nYOUR EMAIL OTP CODE VERIFIER IS: ${emailOtpCode}\nEXPIRES IN: 15 minutes\n======================================================\n`);

    return res.status(403).json({
      error: "Your email address is unverified. Verification required.",
      unverified: true,
      email: user.email,
      otp: REQUIRE_OTP_VERIFICATION ? emailOtpCode : undefined // hide code from client if not enabled
    });
  }
  ...
  // Add to active sessions (allows logout from all devices and expiration handling!)
