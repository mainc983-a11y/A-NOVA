...
export default function App() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (activeSessionId === id) {
          const updatedList = sessionsRef.current.filter(s => s.id !== id);
  ...
  }, [token, activeSessionId, loadSessionDetails]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (activeSessionId && ids.includes(activeSessionId)) {
          const updatedList = sessionsRef.current.filter(s => !ids.includes(s.id));
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (archived && activeSessionId === id) {
          const remaining = sessionsRef.current.filter(s => s.id !== id && !s.archived);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (assistantMsg && assistantMsg.content) {
          ...
          }, 35); // 35ms reduces total paint operations by 40% while preserving ultra-fast responsive text entry
        } else {
          setSessions(prev => 
  ...
  const handleNewSessionPreset = useCallback((mode?: 'general' | 'math' | 'coding' | 'project') => {
    triggerNewSession(mode || 'general');
  }, [triggerNewSession]);
