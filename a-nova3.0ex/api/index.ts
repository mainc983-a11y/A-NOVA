  ...
  try {
    ...
    if (token && token.length >= 3) {
      ...
      const fallbackUser = {
        id: token,
        token: token,
        email: isGuest ? "guest@a-nova.workspace" : `${token}@a-nova.workspace`,
        username: isGuest ? "guest_user" : "user_" + token.slice(0, 8),
...
app.get("/api/auth/me", authenticate, (req, res) => {
  try {
    ...
    return res.json({
      ...
      planStatus: user.planStatus || "Plus" // Default user to "Plus" subscription
    });
  } catch (err: any) {
...
app.post("/api/chats/:id/message", authenticate, async (req, res) => {
  ...
  try {
    ...
    const dynamicSystemPrompt = (userSettings.systemPrompt || "You are A-NOVA, a warm, highly intelligent, and conversational AI companion.") + modeInstruction + customInstructions + humanPersonalityDirective;
